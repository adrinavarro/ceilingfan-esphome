...
# ESPHome derives an entity's object_id from its name with the same rule.
...
"""
            ...
            for label in command_names:
                ...
                button_entries.append(
                    f"""  - platform: template
    id: {button_id}
    name: {current_profile.name} {friendly_label}
    on_press:
      - script.execute:
          id: transmit_command
          frequency: {frequency}
...
def validation_steps(profiles: Sequence[DeviceProfile]) -> list[ValidationStep]:
    ...
    Mirrors the entity naming in render_firmware; ESPHome derives object_ids
    from entity names with the same slug rule as _identifier. The consistency
    test in tests/test_esphome.py guards against the two drifting apart.
    """
    ...
    for profile in profiles:
        ...
        if profile.protocol is not None:
            ...
            steps.append(
                ValidationStep(
                    ...
                    entity_type="button",
                    object_id=_identifier(sync_name),
                )
            )
        if "fan_off" in command_names and speed_keys:
            fan_object = _identifier(fan_name)
            for speed, key in speed_keys:
                steps.append(
                    ValidationStep(
                        profile_name=profile.name,
        ...
        if brightness_keys:
            max_brightness = max(level for level, _ in brightness_keys)
            light_object = _identifier(light_name)
            for level, key in brightness_keys:
                steps.append(
                    ValidationStep(
                        profile_name=profile.name,
            ...
            for label, friendly_label in relative_buttons:
                ...
                steps.append(
                    ValidationStep(
                        ...
                        command=label,
                        entity_type="button",
                        object_id=_identifier(button_name),
                    )
                )
        else:
            for label in command_names:
                ...
                steps.append(
                    ValidationStep(
                        ...
                        command=label,
                        entity_type="button",
                        object_id=_identifier(f"{profile.name} {friendly_label}"),
                    )
                )
    return steps
